...
public class HopLeftHandler extends DrudgerySavingHandler {

	public HopLeftHandler() {
		super("Hop Left", "Hopping Left");
	}
	...
	protected Map<EPlanElement, Date> getChangedTimes(EList<EPlanElement> elements) {
		...
		Date newStartTime = getNewStartTime(elements);
		if (newStartTime != null) {
			map.put(elements.get(0), newStartTime);
		}
		return map;
	}
	...
	private Date getNewStartTime(List<? extends EPlanElement> selectedElements) {
		...
		if (element instanceof EActivity){
			...
			for (EPlanChild ePlanChild : children) {
				if (ePlanChild instanceof EActivity) {
					...
					if (start != null) {
						...
						if (start.before(selectedEnd)) {
							endList.add(start);
						}
					}
					if (end != null) {
						if (end.before(selectedStart)) {
							startList.add(end);
						}
						if (end.before(selectedEnd)) {
							endList.add(end);
						}
					}
				}
			...
			Date startDate = null;
			if (startList.size() > 0) {
				startDate = startList.get(startList.size() - 1);
			}
			Date endDate = null;
			if (endList.size() > 0) {
				endDate = endList.get(endList.size() - 1);
			}
			if (startDate != null && endDate != null) {
				if (selectedStart.getTime() - startDate.getTime() 
						<= selectedEnd.getTime() - endDate.getTime()) {
					result = new Date(startDate.getTime());
				}
				else if (selectedStart.getTime() - startDate.getTime() 
						> selectedEnd.getTime() - endDate.getTime()) {
