 ...
 */
public class NumericResourceTranslator 
	implements ADTranslator 
{
		...
		for (EActivityDef activityDef : activityDefs) {
			...
			for (ENumericResourceEffect effect : activityDef.getNumericEffects()) {
				...
				if (effect.getMode() == ENumericResourceEffectMode.DELTA) {
					are.effects.add(new ResourceEffect(
						effect.getDefinition(),
						effect.getStartEffect(),
						effect.getEndEffect()));
				}
				else {
					LOGGER.severe(
							"AD-NDDL translator can only deal with DELTA effects.\n"+
	...
	for(ActResourceEffect are : actResourceEffects_) {
		...
		for (ResourceEffect effect : are.effects) {
		    if ((effect.startEffect != null)
			&& (effect.startEffect.indexOf(identifier) != -1))
			return true;
		    if ((effect.endEffect != null)
			&& (effect.endEffect.indexOf(identifier) != -1))
			return true;
		}
	    }
	}
    ...
    protected void writeExtraParameters(PrintStream out, EActivityDef activityDef)
    {
    	...
    	params = activityDef.getEStructuralFeatures();
    	for (EStructuralFeature param : params) {
    		if ((param instanceof EAttributeParameter)
		    && formulaUsesIdentifier(activityDef, param.getName())) {
	...
	if (value instanceof Amount<?>) {
		...
		if (val.isExact()) {
		    out.printf("int  \t %s = \t %d; // %s\n",
			       NDDLUtil.escape(constant.getName()),
			       val.getExactValue(),
			       val.getUnit().toString()
			       );
		} else {
		    out.printf("float  \t %s = \t %f; // %s\n",
			       NDDLUtil.escape(constant.getName()),
			       val.getEstimatedValue(),
			       val.getUnit().toString()
			       );
		}
		return;
	...
	protected ST createCompat(
			EActivityDef activity,
	...
	static final String objInstanceTemplate = 
		"<className> NR_<className> = new <className>(<initCap>,<maxCap>);\n"
	;
	
	static final String startCompatTemplate =
		"<className>::<actName> {\n" +
		"  if (scheduled == true\n" +
	...
	static final String endCompatTemplate =
		"<className>::<actName> {\n" +
	        "  if (scheduled == true\n" +
                "       && Enable_Active_Enforcement == true\n" +
                "       && subSolved == true\n" +
