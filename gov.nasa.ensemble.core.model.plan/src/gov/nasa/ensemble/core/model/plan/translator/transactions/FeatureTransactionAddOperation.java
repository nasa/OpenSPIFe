...
public class FeatureTransactionAddOperation<T> extends AbstractTransactionUndoableOperation {

	private final EcoreEList<T> list;
	private final T object;
	private int index = -1;

	public FeatureTransactionAddOperation(String label, EcoreEList<T> list, T object) {
		super(label);
		this.list = list;
		this.object = object;
	}
	...
	protected void undo() throws Throwable {
		if (index != -1) {
			TransactionUtils.writing(list, new Runnable() {
				public void run() {
					if (list.remove(index) != object) {
						LogUtil.error("unexpected object removed");
					}
				}
			});
		}
	...
	public static <T> void execute(EcoreEList<T> list, T object) {
		String label = "add to " + list.getEStructuralFeature().getName();
