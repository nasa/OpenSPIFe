...
public class TimestampedEMFPlanSelectionExportWizardPage extends TimestampedFileSelectionPage {

	public TimestampedEMFPlanSelectionExportWizardPage(int style,  EPlan plan) {
		super(style);
		this.plan = plan;
	}
...
//			selectionButton.setToolTipText(TOOLTIP_FOR_SELECTION);
		...
		TemporalExtent extent = PlanEditorUtil.getPlanExportExtentPropertyValue(this.plan);
...
//		rangeButton.setToolTipText(TOOLTIP_FOR_RANGE);
		...
		fromField = new EnsembleTextBasedDateEditor(this, container, extent.getStart());
...
//		toLabel.setToolTipText(TOOLTIP_FOR_RANGE);
		...
		toField = new EnsembleTextBasedDateEditor(this, container, extent.getEnd());
	...
	public List<EActivity> getActivitiesToExport() {
		...
		if(isSelectionSelected()) {
			...
			for(EActivity activity : activities) {
				...
				if((activityEnd != null && activityEnd.compareTo(startBound) >= 0
						&& activityStart != null && activityStart.compareTo(endBound) < 0)) {
