...
import gov.nasa.ensemble.core.plan.editor.IResourceModelingController;
import gov.nasa.ensemble.core.plan.resources.ResourceUpdater;

public class ResourceModelingController implements IResourceModelingController{

	@Override
	public void enableAutomaticResourceModeling(EPlan plan, boolean enable) {
		ResourceUpdater resourceUpdater = WrapperUtils.getMember(plan, ResourceUpdater.class);
		resourceUpdater.setQueueEnabled(!enable);
		PlanAdvisorMember planAdvisorMember = PlanAdvisorMember.get(plan);
		planAdvisorMember.setListening(enable);
	}
