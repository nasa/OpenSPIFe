...
public class Parser<I, A, E> {
  ...
  public static <I, E> Parser<I, Unit, E> nilParser() {
	  return parser(new F<I, Validation<E, Result<I, Unit>>>() {
		@Override
		public Validation<E, Result<I, Unit>> f(final I input) {
			return success(result(input, unit()));
		}
	});
  }
  
  public <B> Parser<I, A, E> skip(final Parser<I, B, E> parser) {
	  return bind(parser, Function.<B, A>constant());
  }
  
  public Parser<I, Option<A>, E> optional() {
	  return map(Option.<A>some_()).or(Parser.<I, E>nilParser().map(Options.<A>unitToOption()));
  }
  // </ADDED>
  ...
  public Parser<I, Stream<A>, E> repeat1() {
	  ...
	  return bind(new F<A, Parser<I, Stream<A>, E>>() {
		...
		public Parser<I, Stream<A>, E> f(final A a) {
			  return repeat().map(Stream.<A>cons_().f(a));
		  }
	  });
	  // </MODIFIED>
  }
